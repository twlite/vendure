...
to notify on new jobs. It is used by the [SqlJobQueueStrategy](/reference/typescript-api/job-queue/sql-job-queue-strategy#sqljobqueuestrategy) and [InMemoryJobQueueStrategy](/reference/typescript-api/job-queue/in-memory-job-queue-strategy#inmemoryjobqueuestrategy).
